...
import { ArrowRight, Building, ChevronRight, Plus, Search, Trash2, X } from 'lucide-react';
...
export const CompanySelector: React.FC = () => {
  ...
  const filtered = companies.filter(
    (company) =>
      company.razonSocial.toLowerCase().includes(search.toLowerCase()) || company.rut.includes(search)
  );
  ...
  return (
    <Modal
      ...
      overlayClassName="bg-slate-900/90 backdrop-blur-md"
    >
      <div className="bg-white rounded-3xl shadow-2xl w-full overflow-hidden flex flex-col max-h-[85vh] animate-in zoom-in-95">
        <div className="bg-slate-50 p-6 border-b border-slate-200 flex justify-between items-center">
          <div>
            <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight">Maestro de Empresas</h2>
            <p className="text-xs text-slate-500 font-medium">Seleccione o cree una entidad para iniciar el analisis</p>
          </div>
          <button onClick={() => setShowSelector(false)} className="p-2 hover:bg-slate-200 rounded-full transition-colors">
            ...
            <div className="p-8 space-y-6 animate-in slide-in-from-bottom-4">
              <div className="bg-blue-50 p-6 rounded-2xl border border-blue-100 flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-blue-700">Ingrese los datos basicos para crear el contenedor de datos</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormInput
                  ...
                  inputClassName="border-2 border-slate-100 rounded-xl p-3 focus:border-blue-500 font-mono"
                />
                <FormInput
                  label="Razon Social"
                  type="text"
                  ...
                  inputClassName="border-2 border-slate-100 rounded-xl p-3 focus:border-blue-500"
                />
              </div>
            ...
            <>
              ...
              <div className="flex-grow overflow-y-auto p-4 space-y-2">
                {filtered.map((company) => (
                  ...
                  >
                    <div className="flex items-center gap-4 flex-grow" onClick={() => {
                      ...
                      <div className="truncate pr-4">
                        <div className="flex items-center gap-2">
                          ...
                          {company.id.includes('sample') && (
                            <span className="bg-amber-100 text-amber-700 text-[8px] font-black px-1.5 py-0.5 rounded">
                              DEMO
                            </span>
                          )}
                        </div>
                        <p className="text-[10px] text-slate-400 font-mono mt-0.5">{company.rut}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className={`p-2 rounded-lg transition-all ${
                          currentId === company.id ? 'text-blue-600' : 'text-slate-300 group-hover:text-blue-500'
                        }`}
                      >
